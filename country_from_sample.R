...
# Run this the first time, but since there's no way to set the seed for replicability with slice_sample,
# Use the commented-out code below to recreate the original df from the saved mids object once it has been generated
...
allcountry_pmm_by_pv_c <- lapply(allcountry_sample_by_pv_c, impute_data, n_imp, n_iter)
...
save(allcountry_pmm_by_pv_c, file = "models/no_repwt/allcountry_sample_mids_c.rda")
...
country_fromsample_by_pv <- function(specific_country_data) {
  ...
  mids_country <- parLapply(cl, 1:10, function(i) {
    mice.mids(allcountry_pmm_by_pv_c[[i]],
...
for (c in unlist(data_sample_small_c |> distinct(Country))) {
  model_country_allsample <- country_fromsample_by_pv(data_sample_small_c |> filter(Country == c))
  saveRDS(model_country_allsample,
          file = paste0("models/fromsamp/", c, "_small_allsample.rds"))
}

for (c in unlist(data_sample_small_c |> distinct(Country))) {
  model_country_alone <- impute_country_with_each_pv(data_sample_small_c |> filter(Country == c), n_imp = 5)
  saveRDS(model_country_alone,
          file = paste0("models/fromsamp/", c, "_small_alone.rds"))
}

for (c in unlist(data_sample_small_c |> distinct(Country))) {
  model_noimpute <- use_pvs_noimpute(data_sample_small_c |> filter(Country == c))
  saveRDS(model_noimpute,
