...
impute_country_with_each_pv <- function(country_data_numeric, n_imp, n_iter=20) {
  ...
  print("Imputing...")
  weighted_pmm_by_pv <- lapply(data_by_pv,
...
combined_indiv_imputations <- rbind(
  cbind(do.call(rbind, lapply(model_us$imputations, function(x)complete(x, action="long"))) |>
          convert_bg_vars_factor() |>
          mutate(`.imp` = unlist(lapply(1:(n_imp*10), function(x)rep(x,nrow(data_us_small_numeric))))), 
        data.frame(Country="USA")),
  cbind(do.call(rbind, lapply(model_bra$imputations, function(x)complete(x, action="long"))) |>
          convert_bg_vars_factor()|>
          mutate(`.imp` = unlist(lapply(1:(n_imp*10), function(x)rep(x,nrow(data_bra_small_numeric))))),
        data.frame(Country="BRA")))

combined_indiv_imputations <- split(combined_indiv_imputations, combined_indiv_imputations$.imp)
fit_combined_from_indiv <- lapply(combined_indiv_imputations,
                             function(x){
                               svyglm(plausible_FLIT~Gender+Books.Home+Home.Cars+Home.Computer+Siblings+
                                        Immigrant+Father.Ed+Familiar.Fin.Concept+Home.Devices+Grade.Repeat, 
...
weighted_pmm_combined_data <- lapply(data_combined_by_pv,
...
weighted_pmm_combined_qqq <- lapply(data_combined_with_qqq,
                                        impute_data, n_imp)
